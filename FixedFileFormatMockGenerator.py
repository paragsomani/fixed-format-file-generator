...
def getEnumValue(_prop_col_enum_values):
    val = fake.random_choices(elements=(tuple(_prop_col_enum_values)), length=1)
    return val[0] 
...
# starting time
start = time.time()
f_config = open("C:\\Parag\\d\\projects\\mediation\\simulator\\input_config.json","r")
...
filename="c:/tmp/"+name_object+".dat"
f = open(filename, "w")
faker_string = ''
isFirstRun = True
for _ in range(_count):
  ...
  if len(faker_string) != 0:
      ...
      f.write("\n")
  if len(faker_string) != 300 and len(faker_string) != 0:
  ...
  for _prop_item in _properties_array:
      ...
      if _prop_col_data_type =="string":
        stringValue = fake.fixed_width(data_columns=[(_prop_col_width, 'name')], align='right', num_rows=1)
